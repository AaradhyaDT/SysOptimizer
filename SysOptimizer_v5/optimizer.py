...
class GraphCanvas(tk.Canvas):
    ...
    def _schedule_draw(self):
        if self._draw_id is None:
            self._draw_id = self.after_idle(self._deferred_draw)
...
class OptimizerTab(ctk.CTkFrame):
    ...
    def _build_power_section(self, parent):
        ...
        sub = ctk.CTkLabel(card, text="Controls CPU throttling and fan behaviour. Silent = quiet fans, lower power.\n"
                           "Ultimate Performance (CTT) removes micro-latency throttling for maximum responsiveness.",
                           font=("Consolas", zf(9)), text_color=T["fg_dim"])
    ...
    def _set_power(self, guid, label):
        def _do():
            # For Ultimate Performance: provision it first if not present
            if guid == "e9a42b02-d5df-448d-aa00-03f14749eb61":
                run_ps(f"powercfg /duplicatescheme e9a42b02-d5df-448d-aa00-03f14749eb61 2>$null; "
...
class SysOptimizer(ctk.CTk):
    ...
    def _fetch_perf(self):
        ...
        try:
            ...
            net = None
        # Only read disk I/O when perf tab is active to reduce overhead
        try:
    ...
    def _update_perf(self, cpu_total, per_core, ram, swap, freq, net, disk_io):
        ...
        # Only push graph data when the performance tab is visible
    ...
    def _update_disk_sidebar(self):
        # Only update disk sidebar when on processes tab (it's in the sidebar there)
...
if __name__ == "__main__":
    ...
    # Improve scroll responsiveness on Windows — bind mousewheel at root level
    def _on_mousewheel(event):
        widget = event.widget
        # Walk up the widget tree to find a scrollable frame canvas
        try:
            while widget:
